...
mwan3_rtmon_route_handle()
{
	...
	if [ "$route_line" == "$1" ]; then
		...
		# can get route updates on ipv6 where route is already in the table
                # can be deleting table that is already flushed
		[ $action = "add" ] && [ -z "${tbl##*$route_line*}" ] && continue
                [ $action = "del" ] && [ -n "${tbl##*$route_line*}" ] && continue
		network_get_device device "$section"
		$LOG notice "adjusting route $device: $IP route "$action" table $tid $route_line"
...
mwan3_route_line_dev()
{
	...
	local section cfgtype enabled device route_line route_device
        local family route_family _tid
	route_line=$2
        route_family=$3
