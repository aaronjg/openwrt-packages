...
mwan3_update_dev_to_table()
{
	local _tid section family cfgtype curr_table
	mwan3_dev_tbl_ipv4=" "
	mwan3_dev_tbl_ipv6=" "
...
mwan3_route_line_dev()
{
	...
	local section cfgtype enabled device route_line route_device
	local family route_family
	local _tid
	local entry curr_table
	route_line=$2
	route_family=$3
...
mwan3_get_src_ip()
{
	local family _src_ip INTERFACE
	INTERFACE=$2
	unset "$1"
	config_get family $INTERFACE family ipv4
	if [ "$family" = "ipv4" ]; then
		network_get_ipaddr _src_ip ${INTERFACE}
		[ -n "$_src_ip" ] || _src_ip="0.0.0.0"
	elif [ "$family" = "ipv6" ]; then
		network_get_ipaddr6 _src_ip ${INTERFACE}
		[ -n "$_src_ip" ] || _src_ip="::"
	fi
	export "$1=$_src_ip"
}
...
{
	local id via metric V_ IP family
	local iface device cmd

	iface=$1
	device=$2
	config_get family "$iface" family ipv4 device
	...
	network_get_gateway${V_} via "$iface"

	( [ -z "$via" ] || [ "$via" = "0.0.0.0" ] || [ "$via" = "::" ] ) && unset via

	network_get_metric metric "$iface"
...
{
	local tid route_line family IP family id
